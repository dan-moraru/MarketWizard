...
class DataCollector:
    ...
    def register(self) -> bool:
        ...
        try:
            stop_url = f"{self.http_proto}://{self.host}/api/replays/{self.scenario}/stop"
    ...
    def connect(self) -> bool:
        try:
            ...
            for _ in range(50): # Wait up to 2.5s total (50 * 0.05)
                if (self.market_ws.sock and self.market_ws.sock.connected and 
                    self.order_ws.sock and self.order_ws.sock.connected):
                    print(" Connected!")
                    return True
                time.sleep(0.05)
    ...
    def _on_market_data(self, ws, message: str):
        try:
            ...
            if self.current_step % 50 == 0 or self.current_step < 10:
                print(f"[{self.scenario}] Step {self.current_step} | Collected {len(self.data_buffer)} rows...")
    ...
    def save_data(self):
        ...
        try:
            ...
            filename = f"training_data_{self.scenario}_{timestamp}.parquet"
            df.to_parquet(filename, index=False)
            print(f"SUCCESS: Data saved to {filename}")
        except Exception as e:
            print(f"ERROR SAVING DATA: {e}")
            try:
                filename = f"training_data_{self.scenario}_{timestamp}.csv"
    ...
    def run(self):
        ...
        print(f"[{self.student_id}] STARTING COLLECTION. Press Ctrl+C to finish and save.")
        try:
            while self.running:
            ...
            if self.order_ws: self.order_ws.close()
            self.save_data()
