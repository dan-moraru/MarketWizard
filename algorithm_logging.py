...
class TradingBot:
    ...
    def _on_market_data(self, ws, message: str):
        ...
        try:
            ...
            # ================================
            # FEATURE ENGINEERING FOR ML ---> LOGGING
            # ================================
            self.mid_history.append(self.last_mid)
            self.spread_history.append(self.last_ask - self.last_bid)

            if len(self.mid_history) > 50:
                self.mid_history.pop(0)
                self.spread_history.pop(0)

            # Only log once we have enough history
            if len(self.mid_history) >= 10:
                import numpy as np

                mid_returns = np.diff(self.mid_history)

                features = {
                    "spread": self.spread_history[-1],
                    "mid_return": mid_returns[-1],
                    "abs_return": abs(mid_returns[-1]),
                    "volatility_10": np.std(self.mid_history[-10:]),
                    "volatility_50": np.std(self.mid_history),
                    "spread_mean_10": np.mean(self.spread_history[-10:]),
                    "spread_std_10": np.std(self.spread_history[-10:]),
                    "price_acceleration": (
                        mid_returns[-1] - mid_returns[-2]
                        if len(mid_returns) > 1 else 0
                    ),
    ...
    def run(self):
        ...
        try:
            ...
            # ================================
            # SAVE DATASET FOR ML TRAINING ---> LOGGING
            # ================================
            if self.feature_rows:
                import pandas as pd

                df = pd.DataFrame(self.feature_rows)
                filename = f"market_data_{self.scenario}.csv"
                df.to_csv(filename, index=False)

                print(f"\n[{self.student_id}] Saved dataset to {filename}")
